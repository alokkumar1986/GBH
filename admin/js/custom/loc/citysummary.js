...
function _initpage(){
	...
	success: function(result){
				...
				//$(JSON.parse(result)).each(function(i,data){
					...
					_btn1 = '<button class="btn btn-primary editcitybttn">Edit</button>' ;
					_btn2 = '<button class="btn btn-danger deletecitybttn" style="margin-left:1%">Delete</button>' ;
					_td7= $('<td></td>').append(_btn1).append(_btn2);
					
					_tr = $('<tr></tr>').append(_td0).append(_td1).append(_td2).append(_td6).append(_td7);
					$('#locsumm').append(_tr);
			    	}
				});
...
function _navLocDetails(){
	$(document).on('click','.editcitybttn',function(e){
		e.preventDefault();		
		var cityid = $(this).closest('tr').children('td:first').text();
		location.href = 'home.php?page=cityview&id='+cityid;
